...
def main():
    ...
    print("\n\nThis script finds mod conflicts in the main items.json file and extracts them into the custom format for my mods.\n\n")
    ...
    for itemID in itemIDs: #For each itemID that we are looking for
        ...
        for item in items: #For every item in items.json
            ...
            try: conflictingItems = items[item]["_props"]["ConflictingItems"]
            #If not, skip it
            ...
            for conflictingItem in conflictingItems: #For each conflicting item
                ...
                if (conflictingItem == itemID): #If one of the conflicting items matches the one we are looking for
                    ...
                    topLevelItem = items[item]["_id"] #ID of the top-level item in items.json
                    ...
                    finalDict[itemID].append(topLevelItem)
